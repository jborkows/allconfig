...
echo "${repo}" | xargs -I {} gh run list --repo {} --json  databaseId,name,url,status,createdAt,conclusion --limit 100 |\
	...
	--bind="f2:execute(for item in {+}; do id=\$(echo \${item}|cut -f2 -d';'|sed 's/\t/ /'|base64 --decode|jq -r '.databaseId'); echo \${id}; gh api -X DELETE /repos/${repo}/actions/runs/\${id};done;)+abort" \
	--bind="f3:execute:for item in {+}; do id=\$(echo \${item}|cut -f2 -d';'|sed 's/\t/ /'|base64 --decode|jq -r '.databaseId'); echo \${id}; gh api -X DELETE /repos/${repo}/actions/runs/\${id};done;" \
	--bind="f4:execute:for item in {+}; do echo \$(echo \${item}| cut -d';' -f2|sed 's/\t/ /'| base64 --decode); done;" \
