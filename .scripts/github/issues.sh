...
echo "${repo}" | xargs -I {}  gh issue list --json id,labels,state,url,milestone,createdAt,updatedAt,assignees,author,title,number --repo {} --limit 100 |
        ...
        --bind="ctrl-a:execute(echo {}|cut -d$'\t' -f2|base64 --decode|jq -r '.number'|xargs -I % gh issue edit --repo ${repo} --add-assignee \"@me\" %)+abort" \
	--bind="ctrl-e:become:echo {}|cut -d$'\t' -f2|base64 --decode|jq -r '.number'|xargs -I % echo edit % " \
	--bind="F4:abort+become:echo {}|cut -d$'\t' -f2|base64 --decode|jq -r '.number'|xargs -I % echo comment % " \
	--bind="ctrl-q:become:echo {}|cut -d$'\t' -f2|base64 --decode|jq -r '.number'|xargs -I % echo close % " \
        --bind="enter:become(echo {}|cut -d$'\t' -f2 |base64 --decode| jq -r '.url' | xargs -I % brave-browser % )+abort";
